...
func Response_chat(
	...
	cancels *config.Circle_cancels) func(ctx *gin.Context) {
	if typ == "url_chat_chatroom_ctcheck" {
		...
		return func(ctx *gin.Context) {
			chatroom_contact(ctx, sql, cncts_manage, Secret, upgrader)
		}
		...
		return func(ctx *gin.Context) {
			...
			if action == "join" {
				...
				chatroom_manage_create(ctx, sql, cncts_manage, cancels, Secret)
			} else if action == "dissolve" {
				chatroom_manage_dissolve(ctx, sql, cncts_manage, cancels, Secret)
			} else if action == "check" {
				chatroom_manage_check(ctx, sql, Secret)
			} else if action == "exit" {
				chatroom_manage_exit(ctx, sql, cncts_manage, cancels, Secret)
			} else if action == "kick" {
				chatroom_manage_kick(ctx, sql, cncts_manage, Secret)
			} else if action == "role" {
				chatroom_manage_role(ctx, sql, Secret)
			} else {
...
func chatroom_contact(ctx *gin.Context,
	...
	sql.Table("users").
		Where("ID = ?", user_id).
		First(&user)
	go webskt.Read_worker(cncts_manage, user_id, circle_id, sql, user.Name)
}
...
func chatroom_manage_create(ctx *gin.Context,
	...
	cncts_manage.Unlock()
	cancels.Lock()
	ctex, cancels.Cancels[circle.Circle_ID] = context.WithCancel(context.TODO())
	go webskt.Circle_subscrb(circle.Circle_ID, ctex,
		fmt.Sprintf("Circle_%d", circle.Circle_ID), cncts_manage)
	fmt.Println(fmt.Sprintf("Circle Sub(%d) Started", circle.Circle_ID))
	cancels.Unlock()
	normal_tip(&fdbk, ctx, fd)
}
...
func chatroom_manage_dissolve(ctx *gin.Context,
	...
	cncts_manage.Unlock()
	cancels.Lock()
	cancels.Cancels[circle_id]()
	fmt.Println(fmt.Sprintf("Circle Sub(%d) Stopped", circle_id))
	cancels.Unlock()
