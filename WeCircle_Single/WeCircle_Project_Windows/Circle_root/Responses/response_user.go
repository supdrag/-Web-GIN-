...
func Response_user_friends(typ string,
	...
	news_spaces []chan config.Solo_message) func(ctx *gin.Context) {
	if typ == "manage" {
		return func(ctx *gin.Context) {
		...
		return func(ctx *gin.Context) {
			user_friends_contact(ctx, sql, Secret, cncts_manage, upgrader, news_spaces)
		}
	} else if typ == "applies" {
		return func(ctx *gin.Context) {
...
func user_friends_contact(
	...
	cncts_manage *config.Solo_connect_manage,
	upgrader *websocket.Upgrader,
	news_spaces []chan config.Solo_message) {
	var (
		user_id              = controllers.Get_UID(ctx, Secret)
	...
	fmt.Println(fmt.Sprintf("SOLO USING CHANNEL(%d)", chan_id))
	go webskt.Solo_read_worker(cncts_manage, user_id, friend_id, user.Name, news_spaces[chan_id], sql)
}
