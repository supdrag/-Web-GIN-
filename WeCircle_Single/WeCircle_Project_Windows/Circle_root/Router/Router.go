...
func Router_register(r *gin.Engine, sql *gorm.DB, log_lock *sync.Mutex,
	...
	circle_news_spaces []chan config.Circle_message,
	upgrader *websocket.Upgrader) *gin.Engine {
	...
	url_user_friends := url_user.Group("/friends")
	{
		...
		//联系好友
		url_user_friends.GET("/contact", responses.Response_user_friends("contact", sql, Secret, solo_cncts_manage, upgrader, solo_news_spaces))
		//好友管理
		url_user_friends.GET("/manage", responses.Response_user_friends("manage", sql, Secret, solo_cncts_manage, upgrader, solo_news_spaces))
		//好友请求相关
		url_user_friends.GET("/applies", responses.Response_user_friends("applies", sql, Secret, solo_cncts_manage, upgrader, solo_news_spaces))
		//非实时互动
		url_user_friends.GET("/interaction", responses.Response_user_friends("interaction", sql, Secret, solo_cncts_manage, upgrader, solo_news_spaces))
	}
	//====================聊天室路由组====================
	...
	{
		//聊天室查找
		url_chat_chatroom.GET("/ctcheck", responses.Response_chat("url_chat_chatroom_ctcheck", sql, Secret, circle_cncts_manage, upgrader, circle_news_spaces))
		//参与交流
		url_chat_chatroom.GET("/contact", responses.Response_chat("url_chat_chatroom_contact", sql, Secret, circle_cncts_manage, upgrader, circle_news_spaces))
		//交流圈管理
		url_chat_chatroom.GET("/manage", responses.Response_chat("url_chat_chatroom_manage", sql, Secret, circle_cncts_manage, upgrader, circle_news_spaces))
		//交流圈邀请
		url_chat_chatroom.GET("/invitations", responses.Response_chat("url_chat_chatroom_invitations", sql, Secret, circle_cncts_manage, upgrader, circle_news_spaces))
	}
	//====================直播路由====================
